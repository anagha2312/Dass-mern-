...
const browseEvents = async (req, res) => {
	try {
		...
		// Search by name or organizer name (text search)
		if (search) {
			andConditions.push({
				$or: [
		...
		if (
			...
			// Default: show events user is eligible for
			const participantType = user.participantType;
			andConditions.push({
				$or: [
					{ eligibility: "all" },
					{
						eligibility:
							participantType === "iiit" ? "iiit-only" : "non-iiit-only",
					},
				],
			});
		}
...
const registerForEvent = async (req, res) => {
	try {
		...
		if (registration.status === "confirmed") {
			...
			// Send ticket email
			try {
				await sendTicketEmail(registration, event, user);
			} catch (emailError) {
