...
const createOrganizer = async (req, res) => {
	try {
		...
		// Always auto-generate loginEmail: name-iiit@felicity.iiit.ac.in
		const slug = name
			.toLowerCase()
			.replace(/[^a-z0-9]+/g, "-")
			.replace(/^-|-$/g, "")
			.substring(0, 30);
		const loginEmail = `${slug}-iiit@felicity.iiit.ac.in`;
...
const deleteOrganizer = async (req, res) => {
	try {
		...
		if (permanent === "true") {
			// Cascade: delete all events, registrations, discussions, feedback
			const Event = require("../models/Event");
			const Registration = require("../models/Registration");
			...
			const events = await Event.find({ organizer: req.params.id }).select("_id");
			const eventIds = events.map((e) => e._id);

			// Delete all registrations for these events
			await Registration.deleteMany({ event: { $in: eventIds } });

			// Delete all discussions for these events
			...
			// Remove from all participants' followedOrganizers
			const User = require("../models/User");
			await User.updateMany(
				{ followedOrganizers: req.params.id },
				{ $pull: { followedOrganizers: req.params.id } },
			);

			// Permanently delete the organizer
